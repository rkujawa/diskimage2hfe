...
int RAW_libLoad_DiskFile(HXCFLOPPYEMULATOR* floppycontext,FLOPPY * floppydisk,char * imgfile,cfgrawfile * imgformatcfg)
{
	...
	floppycontext->hxc_printf(MSG_DEBUG,"%d bytes sectors, %d sectors/tracks,interleaving %d, %d tracks, %d side(s), gap3 %d, %d rpm, %d bits/s",
		sectorsize,
	...
	for(j=0;j<floppydisk->floppyNumberOfTrack;j++)
	{
		...
		for(i=0;i<floppydisk->floppyNumberOfSide;i++)
		{
			...
			fillindex(currentside->tracklen-1,currentside,2500,1);
			...
			if(tracktype)
			{
				BuildISOTrack(floppycontext,tracktype,currentside->number_of_sector,imgformatcfg->firstidsector,sectorsize,j,i,gap3len,trackdata,currentside->databuffer,&currentside->tracklen,interleave,NULL);
			}
			else
			{
