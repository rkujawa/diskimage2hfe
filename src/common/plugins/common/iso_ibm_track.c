...
int BuildISOTrack(HXCFLOPPYEMULATOR* floppycontext,int TRACKTYPE,unsigned int numberofsector,unsigned char startidsector,unsigned int sectorsize,unsigned int tracknumber,unsigned int sidenumber,unsigned int gap3len,char* datain,unsigned char * mfmdata,unsigned long * mfmsizebuffer,int interleave,SECTORCONFIG * sectorconfigtab)
{
	...
	interleave_tab=compute_interleave_tab((unsigned char)interleave,(unsigned short)numberofsector);
	...
	if(finalsize<=current_buffer_size)
	{
		 		...
		 		for(l=0;l<numberofsector;l++)
		{
				...
				if(sectorconfigtab)
				{
					sectorconfigtab[l].startsectorindex=j*2;
				}
				...
				for(k=0;k<configptr->len_addrmarkp2;k++)
				{
						...
						if(sectorconfigtab)
						{
							if(sectorconfigtab[l].use_alternate_addressmark)
							{
								tempdata[j]=sectorconfigtab[l].alternate_addressmark;
				...
				if(sectorconfigtab)
				{
					tempdata[j]=sectorconfigtab[l].cylinder;
				}
				j++;
				header_size++;
				...
				if(sectorconfigtab)
				{
					tempdata[j]=sectorconfigtab[l].head;
				}
				j++;
				header_size++;
				 				...
				 				if(sectorconfigtab)
				{
					tempdata[j]=sectorconfigtab[l].sector;
					tempclock[j]=0xFF;
				}
				else
				{
				...
				if(sectorconfigtab)
				{
					sectorsize=sectorconfigtab[l].sectorsize;
				}
				...
				for(t=0;t<header_size;t++)  CRC16_Update(&CRC16_High,&CRC16_Low, tempdata[j-header_size+t] );
				...
				if(sectorconfigtab)
				{
					if(sectorconfigtab[l].badheadercrc)
					{
						tempdata[j-1]=(sectorconfigtab[l].header_crc&0xFF);
						tempdata[j]=(sectorconfigtab[l].header_crc>>8)&0xFF;  					}
				}
				j++;
				...
				for(k=0;k<configptr->len_datamarkp2;k++)
				{
						...
						if(sectorconfigtab)
						{
							if(sectorconfigtab[l].use_alternate_addressmark)
							{
								tempdata[j]=sectorconfigtab[l].alternate_datamark;
				...
				if(sectorconfigtab)
				{
					sectorconfigtab[l].startdataindex=j*2;
				}
				...
				if(!sectorconfigtab)
				{
					...
					{
						tempdata[j]=datain[(interleave_tab[l]*sectorsize)+i]; 						tempclock[j]=0xFF;
						j++;
						datapart_size++;
					}
				...
				else
				{
					...
					for(i=0;i<sectorsize;i++)
					{
						tempdata[j]=datain[(indexbuffer)+i]; 						tempclock[j]=0xFF;
						j++;
						datapart_size++;
					}
				...
				for(t=0;t<datapart_size;t++)  CRC16_Update(&CRC16_High,&CRC16_Low, tempdata[j-(datapart_size)+t] );
				...
				if(sectorconfigtab)
				{
					if(sectorconfigtab[l].baddatacrc)
					{
						tempdata[j]++;  					}
				}
				j++;
